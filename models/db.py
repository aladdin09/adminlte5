...
class DefaultConfig:
    def get(self, key, default=None):
        defaults = {
            "db.uri": "postgres://smetadoma03:eY^x7ZQJ1OkQf8Y3g^Z2WvUMv1@localhost:5432/smetadoma03_db?set search_path=public",
            "db.pool_size": 10,
            "db.migrate": True,
...
if "GAE_APPLICATION" not in os.environ:
    ...
    try:
        ...
        # Для PostgreSQL: добавляем обработчик ошибок транзакций
        # Переопределяем execute для автоматического rollback при ошибках
        ...
        def safe_execute(*args, **kwargs):
            try:
                ...
                # Если таблица уже существует - это нормально, игнорируем ошибку
                if "duplicatetable" in error_type.lower() or "already exists" in error_str.lower() or "relation" in error_str.lower() and "already exists" in error_str.lower():
                    # Таблица уже существует - это нормально, делаем commit и возвращаемся
                    try:
                        db.commit()
                    except:
                        try:
                            ...
                            pass
                    return None  # Возвращаем None, так как команда CREATE TABLE не возвращает результат
                
                # Если это ошибка транзакции, откатываем и пробуем еще раз
                if "transaction" in error_str.lower() or "aborted" in error_str.lower() or "25P02" in error_str or "InFailedSqlTransaction" in error_type:
                    try:
                        db.rollback()
                        # Пробуем выполнить команду еще раз после rollback
                        return original_execute(*args, **kwargs)
                    except Exception as retry_err:
                        retry_error_str = str(retry_err)
                        retry_error_type = type(retry_err).__name__
                        # Если при повторной попытке таблица уже существует - это нормально
                        if "duplicatetable" in retry_error_type.lower() or "already exists" in retry_error_str.lower():
                            try:
                                ...
                                try:
                                    ...
                                    pass
                            return None
                        # Если повторная попытка не удалась, откатываем и пробрасываем ошибку
                        try:
                            db.rollback()
                        except:
                            pass
        ...
        # Если ошибка при создании DAL, пробрасываем её
        error_str = str(dal_error)
        if "already exists" in error_str.lower() or "duplicate" in error_str.lower():
            # Попробуем с fake_migrate=True для существующих таблиц
            try:
                db = DAL(
                    configuration.get("db.uri"),
                    pool_size=configuration.get("db.pool_size"),
                    migrate=False,  # Отключаем миграцию при ошибке
                    fake_migrate=False,
                    check_reserved=["all"]
                )
            except:
                # Если и это не помогло, пробрасываем исходную ошибку
                raise dal_error
        else:
            raise
else:
